...
const frame_delay = 15 // ie, 30 fps
...
func GetWinsize() (*winsize, error) {
	...
	ws.Row = uint16(info.Size.Y) / 4
	ws.Col = uint16(info.Size.X) / 4
...
func ClearScreen() {
	...
	err := cmd.Run()
	if err != nil {
		// Если произошла ошибка, выводим её в стандартный вывод
		println("Ошибка очистки экрана:", err.Error())
	}
}
func animate(screen *Screen) {
	...
	for now := range c {
		...
		screen.render(now)
		time.Sleep(time.Millisecond * frame_delay / 2)
		ClearScreen()
	}
}
